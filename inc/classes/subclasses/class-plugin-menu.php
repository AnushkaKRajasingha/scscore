...
class Plugin_menu extends Plugin_Core{
	...
	public function menuInit(){ 
		if (is_admin()) {						
			$admin_page = add_menu_page(
					...
					'dashicons-chart-area' //SCSCORE_PLUGIN_IMGDIR_URL.'/'.self::$current_plugin_data['PluginIcon']
					);
	...
	public function inactiveMenuInit(){
		if (is_admin()) {	
			$admin_page = add_menu_page(
					...
					SCSCORE_PLUGIN_IMGDIR_URL.'/'.self::$current_plugin_data['PluginIcon']*/
			);
	...
	public function _options_page(){
		...
		global $p_set,$p_data;
		extract($page_param);
		include_once SCSCORE_PLUGIN_PAGES.'/page-main.php';
	}
	...
	public function _option_page_sub(){
		if(is_admin() && isset($_REQUEST['page'])){
			...
			$filename = SCSCORE_PLUGIN_PAGES.'/page'.$page_slug.'.php';
 			$page_param = array(
					'p_data' => self::$current_plugin_data,
			...
			if(file_exists($filename))
				...
				include_once SCSCORE_PLUGIN_PAGES.'/page-404.php';
		}
	}
	...
	function _option_page_sub_v1(){
		if(is_admin() && isset($_REQUEST['page'])){
			...
			$filename = SCSCORE_PLUGIN_PAGES.'/page'.$this->key_sub.'.php';
			$page_param = array(
			...
			global $p_set,$p_data,$f_name;
			extract($page_param); var_dump($filename);
			if(file_exists($filename))
				include_once $filename;
			else
				include_once SCSCORE_PLUGIN_PAGES.'/page-404.php';
