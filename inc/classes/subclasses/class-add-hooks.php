...
class Add_Hooks extends Plugin_Core{
	...
	//	if(self::$current_plugin_data['Environment'] == 'Dev'){set_error_handler('var_dump');}
		$this->_addHook();
		register_activation_hook( SCSCORE_PLUGINDIR.'/'.SCSCORE_PLUGIN_FILE,array($this,'_activate'));
		register_deactivation_hook( SCSCORE_PLUGINDIR.'/'.SCSCORE_PLUGIN_FILE,array($this,'_deactivate'));
	...
	public function _activate(){
		//update_option('SCSCORE-settings', 'sadasdasdasd');
		...
		foreach (init_var::_getDbTables() as $key => $value) {
			...
			if($wpdb->get_var("SHOW TABLES LIKE '".$table_name."'") != $table_name){
				$wpdb->query($value); var_dump($value);
			}
		}
	...
	public function mq_plugins_loaded(){
		try {
			...
			if (array_key_exists('Version', $plugin_settings)  &&  self::$current_plugin_data['Version'] > $plugin_settings['Version']) {				
